...
async function findAndMoveImages(markdownFile) {
  ...
  if (parentDir !== fileDir && fs.existsSync(parentDir)) {
    ...
    // Try images named after the markdown file
    for (let i = 1; i <= 10; i++) {
    ...
    // Also try images named after the parent directory (for cases like mein-grovater-rudolf-zeiler/rudolf-zeiler.md)
    for (let i = 1; i <= 10; i++) {
      for (const ext of imageExtensions) {
